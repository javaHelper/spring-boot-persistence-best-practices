...
public interface BookRepository extends JpaRepository<Book, Long> {
    ...
    @Query(value = "SELECT b.title AS title, f.formatType AS formatType FROM Book b, Format f")
    List<BookTitleAndFormatType> findBooksAndFormatsJpql();
    
    // Cross join books and formats (SQL)    
    @Query(value = "SELECT b.title AS title, f.format_type AS formatType FROM book b CROSS JOIN format f",
            nativeQuery = true)
